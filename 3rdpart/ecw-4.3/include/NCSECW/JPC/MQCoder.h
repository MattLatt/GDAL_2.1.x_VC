...
public:
	...
	static NCSJPC_MQ_INLINE INT32 Decode(register UINT32 &AC, register UINT8* &pB, State &S) {
				...
				UINT32 nQEvalShift = S.U.nQEvalShift;
				
				if (((UINT16)AC) >= S.U.Qe.nQEval) {			//(C >= Qe)?
					AC = AC - (S.U.Qe.nQEval + nQEvalShift);	//A-=Qe,C-=Qe

					if((INT32)AC < 0) {							//(A & 0x8000)?
						return((INT8)nD);
					}
					// MPS Exchange
					nNPS = (UINT8)(AC < nQEvalShift);			//NPS = (A < Qe) ? 1 : 0
				} else {										//(C < Qe)
					//LPSExchange
					nNPS = (UINT8)(AC >= 2*nQEvalShift);		//NPS = (A >= 2*Qe) ? 1 : 0
					AC = ((UINT16)AC) + nQEvalShift;			//A = Qe
				}
