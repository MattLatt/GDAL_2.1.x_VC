...
public:
	...
	virtual UINT32 GetNumNodesHigh(UINT16 iComponent = 0) = 0;
		 ...
		 */
	virtual bool GetNode(ReadAccessor &acc_r, UINT32 nNode, UINT16 iComponent = 0) = 0;
	virtual bool GetNode(WriteAccessor &acc_w, UINT32 nNode, UINT16 iComponent = 0) = 0;
		/** 
	...
	virtual bool UnLink(ContextID nCtx, UINT16 nInputs = 0) {
			...
			for(UINT32 n = 0; n < (UINT32)NCSMax(1, nInputs); n++) {
				for(UINT32 i = 0; i < nNodes; i++) {
					WriteAccessor acc_w;
					if(GetNode(acc_w, i, (UINT16)n)) {
						bRet &= acc_w->second.UnLink(nCtx);
					}
				}
			}
			return(CNode2D::UnLink(nCtx));
		};
	...
	virtual bool Read(ContextID nCtx, CBuffer2D *pDst, UINT16 iComponent) {
			...
			if(nNodesHigh && nNodesWide) {
				...
				if(nNodesHigh == 1 && nNodesWide == 1) {
					ReadAccessor acc_r;
					if(GetNode(acc_r, 0, iComponent) && ((CNode2D&)acc_r->second).GetX0() <= nX && ((CNode2D&)acc_r->second).GetX1() >= pDst->GetX1() &&
...
//#define NODETILER2D_READCONST						
						...
						bool bRead = false;
						{
							...
							if(GetNode(acc_r, nNode, iComponent) &&
								bRet &= acc_r->second.Read(nCtx, pDst, iComponent))
							{
								bRet = true;
								bRead = true;
							}
						...
						{
							WriteAccessor acc_w;
							if(GetNode(acc_w, nNode, iComponent)) {
