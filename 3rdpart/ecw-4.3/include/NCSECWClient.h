...
typedef enum {
	...
	/** UINT64 value, max size on disk of local ECWP client cache */
 ...
 *	@return							BOOLEAN value, if this is a local file
 */
extern BOOLEAN NCS_CALL NCSecwNetBreakdownUrlW( wchar_t *szUrlPath,
 ...
 *	@return							NCSError value, NCS_SUCCESS or any applicable error code
 */
extern NCSError NCS_CALL NCScbmOpenFileViewW(wchar_t *szUrlPath, NCSFileView **ppNCSFileView,
					   NCSEcwReadStatus (*pRefreshCallback)(NCSFileView *pNCSFileView));
...
NCSError NCSGetEPSGCode(char *szProjection, char *szDatum, INT32 *pnEPSG);
...
BOOLEAN NCSIsFileInfoExGeoreferenced(NCSFileViewFileInfoEx *pInfo);
	 ...
	 */
void	NCSStandardizeFileInfoEx(NCSFileViewFileInfoEx *pInfo);
...
NCSError NCSGetEPSGCode(char *szDatum, char *szProjection, INT32 *pnEPSG);
	 ...
	 * @return		NCSError		NCS_SUCCESS, or error on failure
	 */
NCSError NCSGetProjectionAndDatum(INT32 nEPSGCode, char **pszProjection, char **pszDatum);
	/**
