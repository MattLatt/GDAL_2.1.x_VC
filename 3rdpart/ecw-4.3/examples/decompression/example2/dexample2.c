...
NCSEcwReadStatus ShowViewCallback(NCSFileView *pNCSFileView)
{
	...
	*/
	...
	printf("Callback blocks: At setView%4d, now%4d, missed%4d, in view%4d\n",
...
void SetViewExtents(MyView *pMyView)
{
	...
	nSize = (UINT32) (nSize * rand() / RAND_MAX);

	if( nSize < MAX_WINDOW )
		nSize = MAX_WINDOW;

	nFromX = (UINT32) (pMyView->pFileInfo->nSizeX - nSize) * rand() / RAND_MAX;
	nFromY = (UINT32) (pMyView->pFileInfo->nSizeY - nSize) * rand() / RAND_MAX;
	nToX = nFromX + nSize;
	nToY = nFromY + nSize;
